...
function EmployeeProfile({employeeId,onClose,onEdit}:{employeeId:string;onClose:()=>void;onEdit:(employee:EmployeeDetail)=>void}){
  ...
  return <div className="profile-layer" onMouseDown={event=>event.target===event.currentTarget&&onClose()}><aside className="employee-profile">
    <header><button className="icon-button" onClick={onClose}><X/></button>{employee&&<><div className="profile-avatar">{employee.profilePhotoUrl?<img src={employee.profilePhotoUrl} alt=""/>:<span>{employee.firstName[0]}{employee.lastName[0]}</span>}</div><h2>{employee.fullName}</h2><p>{employee.jobTitleName||"Team member"} · {employee.employeeNumber}</p><i className={`employee-status ${employee.employmentStatus}`}>{humanize(employee.employmentStatus)}</i><div className="profile-actions"><button onClick={()=>onEdit(employee)}><PencilSimple/>Edit profile</button><button onClick={()=>setStatusOpen(true)}><ClockCounterClockwise/>Change status</button>{employee.profileCompleted&&<button onClick={()=>reopenMutation.mutate()}><UserCircle/>Reopen profile</button>}<button className="archive-action" onClick={()=>window.confirm(`Archive ${employee.fullName}? Their workspace membership will be disabled.`)&&archiveMutation.mutate()}><Archive/>Archive</button></div></>}</header>
...
export function EmployeesPage({onTheme,onMenu}:{onTheme:()=>void;onMenu:()=>void}){
  ...
  return <main className="main-content"><header className="topbar"><button className="mobile-menu icon-button" onClick={onMenu}><List/></button><div className="topbar-copy"><span>Employee directory</span><strong>People records</strong></div><div className="topbar-actions"><button className="theme-trigger" onClick={onTheme}><Palette/>Theme</button><div className="avatar">MK</div></div></header><div className="employees-wrap">
    <section className="page-heading"><div><span className="eyebrow dark"><IdentificationCard/>EMPLOYEE DIRECTORY</span><h1>Everyone, in one place</h1><p>Employment profiles, reporting lines and lifecycle details built on one source of truth.</p></div><div className="heading-actions"><button className="secondary-button" onClick={()=>employeeApi.exportCsv(filters)}><DownloadSimple/>Export</button><button className="primary-button" onClick={()=>setNewOpen(true)}><Plus/>New employee</button></div></section>
    ...
    <nav className="employee-view-tabs"><button className={view==="directory"?"active":""} onClick={()=>setView("directory")}><IdentificationCard/>Directory</button><button className={view==="tree"?"active":""} onClick={()=>setView("tree")}><TreeStructure/>Org tree</button><button className={view==="events"?"active":""} onClick={()=>setView("events")}><Cake/>Upcoming</button><button className={view==="archived"?"active":""} onClick={()=>setView("archived")}><Archive/>Archived</button>{view==="directory"&&<span>{bulkMode?<><button disabled={!selectedIds.length} onClick={()=>setBulkOpen(true)}>Update {selectedIds.length||"selected"}</button><button onClick={leaveBulkMode}>Cancel</button></>:<button onClick={()=>setBulkMode(true)}>Bulk assign</button>}</span>}</nav>
