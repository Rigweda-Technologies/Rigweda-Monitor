...
export function OrganizationsPage({ onTheme, onMenu }: { onTheme:()=>void; onMenu:()=>void }) {
  ...
  return <main className="main-content"><PageHeader onTheme={onTheme} onMenu={onMenu}/><div className="organizations-wrap">
    <section className="page-heading"><div><span className="eyebrow dark"><Buildings size={14}/>WORKSPACE DIRECTORY</span><h1>Your organizations</h1><p>Create and configure each independent people workspace.</p></div>{canManageOrganizations&&<button className="primary-button" onClick={()=>setEditing(null)}><Plus size={16}/>New organization</button>}</section>
