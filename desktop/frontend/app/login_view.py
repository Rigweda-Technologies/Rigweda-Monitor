...
class LoginApp:
    ...
    def _start_monitoring(self, session: dict | None, *, register_windows_startup: bool) -> bool:
        ...
        if not app_usage_started:
            self.signin_button.configure(state="normal", text="Sign In")
            self._set_status(app_usage_message, COLORS["error"])
            return False
        ...
        self._set_status("Login successful. Screenshot, activity, and app usage monitoring are running.", COLORS["success"])
